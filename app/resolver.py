...
def random_genres_items(genre):
    ...
    # Todo 선택한 장르의 갯수가 5보다 적은경우 처리해야함 : 2025.3.31 정우진
    nitem = min(5,genre_df.size) # genre+df.shape[0] : 크기. [1]: 칼럼갯수
...
def random_genres_items_best(genre):
    ...
    genre_df = movies_df[movies_df['genres'].apply(lambda x: genre in x.lower())]
    genre_df = genre_df.fillna('')
    filtered_df = genre_df[genre_df['rcount'] >= 5].sort_values(by='rmean', ascending=False)
    result_items = filtered_df.head(5).to_dict('records')
    return result_items